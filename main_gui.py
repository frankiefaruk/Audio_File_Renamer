    ...
    def rename_audio_files(self):
        ...
        try:
            ...
            if renamed_count > 0:
                ...
                message += "Backup created in 'backup' folder."
                if errors:
                    message += f"\n\nErrors occurred with {len(errors)} files."
                messagebox.showinfo("Rename Complete", message)
            else:
                messagebox.showwarning("No Files Renamed", 
                                     "No files were renamed. Check file naming format.")
            
            if errors:
                error_message = "\n".join(errors)
                messagebox.showerror("Rename Errors", error_message)
            
            # After showing results, reset the button back to Submit
            self.submit_button.config(
                text="Submit",
                command=self.on_submit
            )
