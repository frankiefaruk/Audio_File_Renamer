...
class MidiNoteExtractor:
    ...
    def extract_note_from_filename(self, filename):
        ...
        if midi_match:
            ...
            if midi_num in self.midi_to_note and self.midi_to_note[midi_num] == note_name:
                return midi_num, note_name

        # If no valid MIDI match, search for note name and resolve MIDI number
        note_pattern = re.search(r'([A-G]#?\-?\d+)', filename)
        if note_pattern:
            note_name = note_pattern.group(1)
            midi_num = self.note_mapping.get(note_name)
